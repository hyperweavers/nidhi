...
export class MarketService {
  ...
  private getIndexDetails(code: string): Observable<Index> {
    return this.http.get<IndexQuotes>(Constants.api.INDEX_QUOTES + code).pipe(
      map(
        ({ indicesList }): Index => ({
          complete: true,
          ...
          quote: {
            lastUpdated: MarketUtils.dateStringToEpoch(indicesList[0].dateTime),
            value: indicesList[0].lastTradedPrice,
            change: {
              direction: MarketUtils.getDirection(indicesList[0].percentChange),
