...
import { mockForexResponse, mockCurrencyListResponse } from '../../mocks/data';
...
describe('CurrencyService', () => {
  ...
  describe('getCurrencyList', () => {
    it('should return mapped currency list', (done) => {
      service.getCurrencyList().pipe(take(1)).subscribe((currencies) => {
  ...
  describe('getForexRates', () => {
    it('should return mapped currency matrix on successful response', (done) => {
      service.getForexRates().pipe(take(1)).subscribe((matrix) => {
    ...
    it('should log error and return empty object when success is not 1', (done) => {
      service.getForexRates().pipe(take(1)).subscribe((matrix) => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith('Failed to fetch forex rates!');
        done();
      });
