...
- **Pangu**: `mockServiceWorker.js` in `src/`, test server wired via `test-utils.ts` + `test-setup.ts`
...
To re-generate `mockServiceWorker.js` when updating MSW (pangu keeps its worker in `src/`, the others in `public/`):

```bash
pnpm msw init apps/pangu/src --save
pnpm msw init apps/<app>/public --save   # palan, vatti
```
